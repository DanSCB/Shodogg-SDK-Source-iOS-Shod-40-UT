...
- (ConnectableDevice *) deviceForUUID:(NSString *)UUID
{
    if (!UUID || UUID.length == 0 || [UUID isKindOfClass:[NSNull class]])
        return nil;

    ConnectableDevice *foundDevice = [self activeDeviceForUUID:UUID];

    if (!foundDevice)
    {
        NSDictionary *foundDeviceInfo = [self storedDeviceForUUID:UUID];
