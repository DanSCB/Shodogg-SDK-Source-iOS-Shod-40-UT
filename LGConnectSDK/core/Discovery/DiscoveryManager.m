...
//XXX: Temporary hack because of usage of DIAL service for FireTV platform
- (BOOL)isDialNotFireTV:(ServiceDescription*)description {
    NSString *serviceName = description.serviceId;
    NSString *modelName = description.modelName;
    
    //
    //   Commmenting out these three lines results in AppleTV buttons displaying as device options
    //
    //   if (!modelName) { //FireTV returns name
    //       return YES;
    //   }
    
    if ([serviceName localizedCaseInsensitiveContainsString:@"DIAL"] &&
        ![modelName localizedCaseInsensitiveContainsString:@"FireTV"]) {
...
- (void)discoveryProvider:(DiscoveryProvider *)provider didFindService:(ServiceDescription *)description
{
    ...
    //XXX: Temporary hack because of usage of DIAL service for FireTV platform
    if ([self isDialNotFireTV:description]) {
        return;
    }
    ...
    if (deviceIsNew)
    {
        ...
        {
            device = [self.deviceStore deviceForUUID:description.UUID];
